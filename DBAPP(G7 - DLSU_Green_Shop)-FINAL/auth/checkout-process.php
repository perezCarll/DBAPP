...
if ($currencyId > 0) {
    try {
        $stmt = $pdo->prepare("SELECT currency_id FROM Currencies WHERE currency_id = ?");
        $stmt->execute([$currencyId]);
        if (!$stmt->fetch()) {
            $errors[] = "Invalid currency selected.";
            $currencyId = 1; // Fallback to PHP
        }
    } catch (PDOException $e) {
        $errors[] = "Error validating currency.";
        $currencyId = 1; // Fallback to PHP
    }
}
...
//  
// 4. VALIDATE CART ITEMS & VERIFY PRODUCTS EXIST
//    (We use the total from frontend in selected currency)
//  
$cartItems = [];
...
try {
    foreach ($cart as $item) {
        ...
        // Verify product exists and get base PHP price for Order_Items
        ...
        $unitPricePhp = (float)$row['price']; // Base PHP price

        $cartItems[] = [
            'product_id' => $pid,
            'qty'        => $qty,
            'unit_price' => $unitPricePhp, // Store PHP price in Order_Items
            'name'       => $row['name'],
        ];
    }
} catch (Exception $e) {
    $errors[] = "Error validating cart items.";
}
...
// Use the total from frontend (already converted to selected currency)
$total = $totalAmount;
...
try {
    ...
    $stmt->execute([
        ...
        $total,       // Order total in selected currency
...
//    (Using PHP base prices for consistency)
//  
foreach ($cartItems as $ci) {
    $pid = $ci['product_id'];
    $qty = $ci['qty'];
    $up  = $ci['unit_price']; // PHP base price
