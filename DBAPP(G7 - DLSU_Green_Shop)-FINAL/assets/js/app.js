  ...
  $$(".currency-option").forEach((item) => {
    item.addEventListener("click", (e) => {
      ...
      document.dispatchEvent(
        new CustomEvent("currency:changed", { detail: currentCurrency })
      );
    });
  });
...
function renderProducts() {
  ...
  function applyFiltersAndPrices() {
    ...
    cards.forEach((card) => {
      ...
      const col =
        card.closest(".col-12, .col-sm-6, .col-lg-4") || card.parentElement;
  ...
  grid.addEventListener("click", (e) => {
    ...
    if (e.target.closest(".btn-quick-view")) {
      ...
      if (priceEl)
        priceEl.textContent = formatCurrency(priceVal, currentCurrency);
      if (addBtn) {
...
function updateCartUI() {
  ...
  if (cart.length === 0) {
    ...
    cart.forEach((item, idx) => {
      ...
      const priceNow =
        (item.prices[currentCurrency] ?? item.prices.PHP ?? 0) * item.qty;
      li.innerHTML = `
        ...
        <div class="text-end">
          <div class="fw-semibold">${formatCurrency(
            priceNow,
            currentCurrency
          )}</div>
...
function applyRoleUI() {
  ...
  adminLinks.forEach((link) => {
    ...
    link.title = allowed
      ? ""
      : "Login as Staff/Manager/Admin to access this (demo)";
  });
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (checkoutForm) {
    checkoutForm.addEventListener("submit", (e) => {
      ...
      const curCodeField = $("#checkoutCurrencyCode");
      if (curCodeField) {
        curCodeField.value = currentCurrency;
      }

      // Set currency ID
      const currencyIds = { PHP: 1, USD: 2, EUR: 3 };
      const curIdField = $("#checkoutCurrencyId");
      if (curIdField) {
        curIdField.value = currencyIds[currentCurrency] || 1;
      }

      // Calculate and set total amount in selected currency
      const fulfilmentSel = $("#checkoutFulfilment");
      const fulfilment = fulfilmentSel?.value || "pickup";

      const subtotal = cart.reduce(
        (sum, i) =>
          sum + (i.prices[currentCurrency] ?? i.prices.PHP ?? 0) * i.qty,
        0
      );
      ...
      const totalField = $("#checkoutTotalAmount");
      if (totalField) {
